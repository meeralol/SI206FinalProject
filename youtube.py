...
def make_ratings_table(view_list, like_list, cur, conn):
    ...
    for i in view_list[start: start + 25]:
        cur.execute('INSERT OR IGNORE INTO Ratings (id, views, likes) VALUES (?, ?, ?)',(id, view_list[count], like_list[count])) 
        id += 1
        count += 1
    ...
    #     cur.execute('INSERT INTO Ratings (id, views, likes) VALUES (?, ?, ?)',(id,view_list[i], like_list[i])) 
    #     id += 1
...
make_ratings_table(list_of_views, list_of_likes, cur, conn)
